...
double pulse::period(double w, int cycles) {
  return cycles * 2.0 * M_PI / w;
}

void pulse::gaussESetup(double Io, double w, double tau, int cycles, double cepd,
                        pulse::params& pars) {
  constexpr double width = 0.58870501125773734549;
  pars.Eo = sqrt(Io);
  pars.w = w;
  double To = 2 * M_PI / w;
  int cycles_fwhm = 2 + static_cast<int>(cycles / 10);
  pars.cepd = cepd;
  pars.a = 0.5 * To * cycles_fwhm / width;
  pars.b = tau;
}

void pulse::gaussASetup(double Io, double w, double tau, int cycles, double cepd,
                        pulse::params& pars) {
  constexpr double width = 0.58870501125773734549;
  pars.Eo = sqrt(Io) / w;
  pars.w = w;
...
double pulse::gauss(pulse::params& pars, double t) {
  ...
  return static_cast<double>(t >= 0.0 && t <= pars.b) * pars.Eo * exp(-t_diff * t_diff / (pars.a * pars.a)) *
         sin(pars.w * t + pars.cepd);
}

void pulse::sineASetup(double Io, double w, double cepd, int cycles, pulse::params& pars) {
  pars.Eo = sqrt(Io) / w;
  pars.w = w;
...
double pulse::sineAA(pulse::params& pars, double t) {
  ...
  return static_cast<double>(t >= 0.0 && t <= M_PI / pars.a) * pars.Eo * sinWt * sinWt *
         sin(pars.w * t + pars.cepd);
}
...
double pulse::sineAE(pulse::params& pars, double t) {
  ...
  return static_cast<double>(t >= 0.0 && t <= M_PI / pars.a) * pars.Eo *
         (pars.w * sinWt * sinWt * cos(wtc) + pars.a * sin(2 * Wt) * sin(wtc));
}

void pulse::sineESetup(double Io, double w, double cepd, int cycles, pulse::params& pars) {
  pars.Eo = sqrt(Io);
  pars.w = w;
...
double pulse::sineEA(pulse::params& pars, double t) {
  ...
  return static_cast<double>(t >= 0.0 && t <= M_PI / pars.a) * 0.5 * pars.Eo *
