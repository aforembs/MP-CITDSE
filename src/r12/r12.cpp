...
int Prprimlob4(int n, int bo, int glq_pt,
  ...
  int i1, i1bo, i2bo, sp, ai, bi, ibo1j, jbopi;
  ...
  for(auto i=bo-1; i<n; ++i) {
    ...
    for(auto p=0; p<glq_pt; ++p, sp+=2){
      ...
      for(auto j=0; j<bo; ++j) {
        ...
        Pl1pa += Cf[off1+ai]*Ssp[j+bo*(sp+i*2*glq_pt)];
        Pl1pb += Cf[off1+bi]*Ssp[j+bo*(sp+1+i*2*glq_pt)];
        Pl2pa += Cf[off2+ai]*Ssp[j+bo*(sp+i*2*glq_pt)];
        Pl2pb += Cf[off2+bi]*Ssp[j+bo*(sp+1+i*2*glq_pt)];
      }
      p1p_out[i1bo+p]   =Pl1p;
      p2p_out[i1bo+p]   =Pl2p;
      p1p_in[i2bo+sp]   =Pl1pa;
      p1p_in[i2bo+sp+1] =Pl1pb;
      p2p_in[i2bo+sp]   =Pl2pa;
      p2p_in[i2bo+sp+1] =Pl2pb;
      rm1=r;
    }
  }
...
int r_12::R12Trap(std::string cpot, int L_max, int glq_pt, std::string dir) {
  ...
  for(int L=0; L<=L_max; ++L) {
    ...
    for(int NL2=0; NL2<L_sz; ++NL2) {
      ...
      // calculate P1' P2' here and pass it to sltr
      Prprim(n, bo, glq_pt, e12p.l1*lc_sz+e12p.n1*n, e12p.l2*lc_sz+e12p.n2*n, 
        kkn, gl_x, Bsplines, Cf, p1p_buff, p2p_buff);
      }
      ...
      for(int NL1=NL2; NL1<L_sz; ++NL1) {
        ...
        for(int k=0; k<=k_max; ++k) {
          ...
          if(min_dir ==(((L+e12.l1+e12p.l2) >> 0) & 1) &&
             ...
             !(((k+e12.l2+e12p.l2) >> 0) & 1) &&
             !std::isinf(1.0/rk[(k+1)*n*glq_pt])) {
            sum_k += pow(-1,min_dir)*FsltrTrapGL(k, n, bo, glq_pt, gl_w, 
                      gl_x, kkn, Bsplines, rk, l1i_loc, 
                      l2i_loc, p1p_buff, p2p_buff, pi)
                      *wig3jj(2*e12.l1,2*k,2*e12p.l1,0,0,0)
                      *wig3jj(2*e12.l2,2*k,2*e12p.l2,0,0,0)
                      *wig6jj(2*e12p.l1,2*e12p.l2,2*L,2*e12.l2,2*e12.l1,2*k);
          }
          ...
          if(min_exc ==(((L+e12.l2+e12p.l2) >> 0) & 1) &&
             ...
             !(((k+e12.l2+e12p.l1) >> 0) & 1) &&
             !std::isinf(1.0/rk[(k+1)*n*glq_pt])) {
            sum_k += pow(-1,min_exc)*FsltrTrapGL(k, n, bo, glq_pt, gl_w, 
                      gl_x, kkn, Bsplines, rk, l1i_loc, 
                      l2i_loc, p2p_buff, p1p_buff, pi)
                      *wig3jj(2*e12.l1,2*k,2*e12p.l2,0,0,0)
                      *wig3jj(2*e12.l2,2*k,2*e12p.l1,0,0,0)
                      *wig6jj(2*e12p.l1,2*e12p.l2,2*L,2*e12.l1,2*e12.l2,2*k);
          }
