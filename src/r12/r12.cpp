...
int Rpowk(std::vector<double> &r_out, int n, int bo, int glq_pt,
  ...
  for (auto k = 2; k <= k_max; ++k) {
    ...
    for (auto i = 0; i < n + 1 - bo; ++i) {
      ...
      for (auto p = 0; p < glq_pt; ++p) {
        r_out[p + iqpt + k * nqpt] =
            r_out[p + iqpt + nqpt] * r_out[p + iqpt + km1 * nqpt];
      }
    }
  }
...
int Rpowk(std::vector<double> &r_out, std::vector<double> &r_in, int n, int bo,
          int glq_pt, std::vector<double> &gl_x, std::vector<double> &kkn,
          int k_max) {
  auto nqpt = n * glq_pt;
  r_out.reserve((k_max + 1) * nqpt);
    ...
    for (auto i = 0; i < n + 1 - bo; ++i) {
      ...
      for (auto p = 0; p < glq_pt; ++p) {
        r_out[p + iqpt + k * nqpt] =
            r_out[p + iqpt + nqpt] * r_out[p + iqpt + km1 * nqpt];
        r_in[p + iqpt + k * nqpt] =
            r_in[p + iqpt + nqpt] * r_in[p + iqpt + km1 * nqpt];
      }
    }
  }
...
int Rpowklob4(std::vector<double> &r_out, std::vector<double> &r_in, int n,
              int bo, int glq_pt, std::vector<double> &gl_x,
  ...
  for (auto k = 2; k <= k_max; ++k) {
    ...
    for (auto i = 0; i < n + 1 - bo; ++i) {
      ...
      for (auto p = 0; p < glq_pt; ++p, sp += 2) {
        r_out[p + iqpt + k * nqpt] =
            r_out[p + iqpt + nqpt] * r_out[p + iqpt + km1 * nqpt];
        r_in[sp + iqpt2 + k * nqpt2] =
            r_in[sp + iqpt2 + nqpt2] * r_in[sp + iqpt2 + km1 * nqpt2];
        r_in[sp + 1 + iqpt2 + k * nqpt2] =
            r_in[sp + 1 + iqpt2 + nqpt2] * r_in[sp + 1 + iqpt2 + km1 * nqpt2];
      }
    }
  }
...
int Rpowk(std::vector<double> &r_out, std::vector<double> &r_in, int n, int bo,
          int glq_pt, std::vector<double> &gl_xo, std::vector<double> &gl_xi,
          std::vector<double> &kkn, int k_max) {
...
int r_12::r12MM(std::string cpot, int L_max, int glq_pt, std::string dir) {
  ...
  for (int l = 0; l <= e1_lm; ++l) {
    ...
    cspace = rset->getSpace();
    cspace.selectHyperslab(H5S_SELECT_SET, count, offset, stride, block);
    rset->read(&wfn_i[l * lc_sz], H5::PredType::NATIVE_DOUBLE, memspace,
               cspace);
    file->close();
  }
        ...
        for (int NL1 = NL2; NL1 < L_sz; ++NL1) {
          ...
          for (int k = 0; k <= k_max; ++k) {
            ...
            if (min_dir == (((L + e12.l1 + e12p.l2) >> 0) & 1) &&
              ...
              sum_k += pow(-1, min_dir) *
                       FsltrMM(k, n, bo, glq_pt, lc_sz, e12.n1, e12.l1, e12.n2,
                               e12.l2, e12p.n1, e12p.l1, e12p.n2, e12p.l2, gl_w,
                               gl_x, kkn, rk, rk_mid, wfn_o, wfn_i) *
            ...
            if (min_exc == (((L + e12.l2 + e12p.l2) >> 0) & 1) &&
              ...
              sum_k += pow(-1, min_exc) *
                       FsltrMM(k, n, bo, glq_pt, lc_sz, e12.n1, e12.l1, e12.n2,
                               e12.l2, e12p.n2, e12p.l2, e12p.n1, e12p.l1, gl_w,
                               gl_x, kkn, rk, rk_mid, wfn_o, wfn_i) *
...
int r_12::r12Glob4(std::string cpot, int L_max, int glq_pt, std::string dir) {
  ...
  for (int l = 0; l <= e1_lm; ++l) {
    ...
    rset->read(&wfn_o[l * lc_sz], H5::PredType::NATIVE_DOUBLE, memspace_o,
               cspace);
    rset =
        std::make_unique<H5::DataSet>(H5::DataSet(file->openDataSet("Pr_i")));
    cspace = rset->getSpace();
    cspace.selectHyperslab(H5S_SELECT_SET, count_i, offset, stride, block);
    rset->read(&wfn_i[l * lc_sz], H5::PredType::NATIVE_DOUBLE, memspace_i,
               cspace);
    file->close();
  }
        ...
        for (int NL1 = NL2; NL1 < L_sz; ++NL1) {
          ...
          for (int k = 0; k <= k_max; ++k) {
            ...
            if (min_dir == (((L + e12.l1 + e12p.l2) >> 0) & 1) &&
              ...
              sum_k +=
                  pow(-1, min_dir) *
                  FsltrLob4GL(k, n, bo, glq_pt, lc_sz, e12.n1, e12.l1, e12.n2,
                              e12.l2, e12p.n1, e12p.l1, e12p.n2, e12p.l2, gl_w,
                              gl_x, kkn, rk, rk_mid, wfn_o, wfn_i) *
                  wig3jj(2 * e12.l1, 2 * k, 2 * e12p.l1, 0, 0, 0) *
                  wig3jj(2 * e12.l2, 2 * k, 2 * e12p.l2, 0, 0, 0) *
        ...
        for (int NL1 = NL2; NL1 < L_sz; ++NL1) {
          ...
          for (int k = 0; k <= k_max; ++k) {
            ...
            if (min_dir == (((L + e12.l1 + e12p.l2) >> 0) & 1) &&
              ...
              sum_k +=
                  pow(-1, min_dir) *
                  FsltrLob3GL(k, n, bo, glq_pt, lc_sz, e12.n1, e12.l1, e12.n2,
                              e12.l2, e12p.n1, e12p.l1, e12p.n2, e12p.l2, gl_w,
                              gl_x, kkn, rk, rk_mid, wfn_o, wfn_i) *
                  wig3jj(2 * e12.l1, 2 * k, 2 * e12p.l1, 0, 0, 0) *
                  wig3jj(2 * e12.l2, 2 * k, 2 * e12p.l2, 0, 0, 0) *
        ...
        for (int NL1 = NL2; NL1 < L_sz; ++NL1) {
          ...
          for (int k = 0; k <= k_max; ++k) {
            ...
            if (min_dir == (((L + e12.l1 + e12p.l2) >> 0) & 1) &&
              ...
              sum_k += pow(-1, min_dir) *
                       FsltrTrapGL(k, n, bo, glq_pt, lc_sz, e12.n1, e12.l1,
                                   e12.n2, e12.l2, e12p.n1, e12p.l1, e12p.n2,
                                   e12p.l2, gl_w, gl_x, kkn, rk, wfn_o) *
                       wig3jj(2 * e12.l1, 2 * k, 2 * e12p.l1, 0, 0, 0) *
                       wig3jj(2 * e12.l2, 2 * k, 2 * e12p.l2, 0, 0, 0) *
                       wig6jj(2 * e12p.l1, 2 * e12p.l2, 2 * L, 2 * e12.l2,
                              2 * e12.l1, 2 * k);
            ...
            if (min_exc == (((L + e12.l2 + e12p.l2) >> 0) & 1) &&
              ...
              sum_k += pow(-1, min_exc) *
                       FsltrTrapGL(k, n, bo, glq_pt, lc_sz, e12.n1, e12.l1,
                                   e12.n2, e12.l2, e12p.n2, e12p.l2, e12p.n1,
                                   e12p.l1, gl_w, gl_x, kkn, rk, wfn_o) *
                       wig3jj(2 * e12.l1, 2 * k, 2 * e12p.l2, 0, 0, 0) *
                       wig3jj(2 * e12.l2, 2 * k, 2 * e12p.l1, 0, 0, 0) *
                       wig6jj(2 * e12p.l1, 2 * e12p.l2, 2 * L, 2 * e12.l1,
                              2 * e12.l2, 2 * k);
