...
int w1e::ReadConfig(std::string file, int &glq_pt, int &l_max,
...
int Pr(int n, int bo, int glq_pt, int off, int ooff,  
...
int Prlob3(int n, int bo, int glq_pt, int off, int ooff,
  ...
  for(auto i=bo-1; i<n; ++i) {
    ...
    for(auto p=0; p<glq_pt; ++p){
      ...
      for(auto j=0; j<bo; ++j) {
        Pl += Cf[off+i1-bo+j-(kkn[i]>rlob)]*Ssp[j+bo*(p+i*glq_pt)];
      }
...
int w1e::GenWfn(std::string pot, int glq_pt, int l_max, std::string integrator) {
  int n, k, nen;
