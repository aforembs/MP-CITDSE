...
int dmx2e::readConfig(std::string file, std::string& pot, int& L_max, int& l_max, char& gauge) {
  ...
  std::cout << "Maximum one electron angular momentum:       " << l_max << std::endl;
  return 0;
}

int dmx2e::genDipole(std::string pot, int L_max, int l_max, char gauge, std::string dir) {
...
#pragma omp for private(l1i, l2i, l1f, l2f, n1i, n2i, n1f, n2f)
      for (int idLf = 0; idLf < Lf_sz; ++idLf) { // up to sz Lf
        ...
        if (l1i + 1 == l1f && l2i == l2f && n2i == n2f) {
          Tif += pow(-1, l2i + l1f) * sqrt(4.0 * l1f * l1f - 1.0) *
                 ...
                 D_data[l1i * max2 + n1f + max_N * n1i] / sqrt(static_cast<double>(l1f));
        }
        if (l2i + 1 == l2f && l1i == l1f && n1i == n1f) {
          Tif += pow(-1, l1i + l2f) * sqrt(4.0 * l2f * l2f - 1.0) *
                 wig6jj(2 * l2i, 2 * l1i, 0, 2, 2, 2 * l2f) *
                 D_data[l2i * max2 + n2f + max_N * n2i] / sqrt(static_cast<double>(l2f));
        }
        if (l1i + 1 == l2f && l2i == l1f && n2i == n1f) {
          Tif += pow(-1, l2i + l2f) * sqrt(4.0 * l2f * l2f - 1.0) *
                 wig6jj(2 * l1i, 2 * l2i, 0, 2, 2, 2 * l2f) *
                 D_data[l1i * max2 + n2f + max_N * n1i] / sqrt(static_cast<double>(l2f));
        }
        if (l2i + 1 == l1f && l1i == l2f && n1i == n2f) {
          Tif += pow(-1, l1i + l1f) * sqrt(4.0 * l1f * l1f - 1.0) *
                 wig6jj(2 * l2i, 2 * l1i, 0, 2, 2, 2 * l1f) *
                 D_data[l2i * max2 + n1f + max_N * n2i] / sqrt(static_cast<double>(l1f));
        }
        if (l1i - 1 == l1f && l2i == l2f && n2i == n2f) {
          Tif += dipT * pow(-1, l2i + l1i) * sqrt(4.0 * l1i * l1i - 1.0) *
                 wig6jj(2 * l1i, 2 * l2i, 0, 2, 2, 2 * l1f) *
                 D_data[l1f * max2 + n1i + max_N * n1f] / sqrt(static_cast<double>(l1i));
        }
        if (l2i - 1 == l2f && l1i == l1f && n1i == n1f) {
          Tif += dipT * pow(-1, l1i + l2i) * sqrt(4.0 * l2i * l2i - 1.0) *
                 wig6jj(2 * l2i, 2 * l1i, 0, 2, 2, 2 * l2f) *
                 D_data[l2f * max2 + n2i + max_N * n2f] / sqrt(static_cast<double>(l2i));
        }
        if (l1i - 1 == l2f && l2i == l1f && n2i == n1f) {
          Tif += dipT * pow(-1, l2i + l1i) * sqrt(4.0 * l1i * l1i - 1.0) *
                 wig6jj(2 * l1i, 2 * l2i, 0, 2, 2, 2 * l2f) *
                 D_data[l2f * max2 + n1i + max_N * n2f] / sqrt(static_cast<double>(l1i));
        }
        if (l2i - 1 == l1f && l1i == l2f && n1i == n2f) {
          Tif += dipT * pow(-1, l1i + l2i) * sqrt(4.0 * l2i * l2i - 1.0) *
                 wig6jj(2 * l2i, 2 * l1i, 0, 2, 2, 2 * l1f) *
                 D_data[l1f * max2 + n2i + max_N * n1f] / sqrt(static_cast<double>(l2i));
        }
        if (l1i == l2i && n1i == n2i) {
          Tif *= sq2;
        }
  ...
  dmx = std::make_unique<H5::DataSet>(H5::DataSet(
      outfile->createDataSet("d_if", H5::PredType::NATIVE_DOUBLE, H5::DataSpace(2, T_dimms))));
