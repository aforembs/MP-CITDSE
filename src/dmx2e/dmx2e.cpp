...
int DMX2e::sort_L(uint L_max, int ncf, std::vector<int> &cfgs) {
  uint l1=0;
  uint l2=0;
  std::string filename;
  std::string outfile_name;
  ...
  std::vector<cfg_line> cfg_st(ncf);
  int t_sz=0;
  for(auto i=0; i<ncf; ++i) {
    min_n2 = cfgs[i*5+3];
    max_n2 = cfgs[i*5+4];
    cfg_st[i] = {cfgs[i*5],cfgs[i*5+1],cfgs[i*5+2],min_n2,max_n2};
    t_sz += max_n2-min_n2+1;
  }
