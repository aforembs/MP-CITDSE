...
int DMX2e::sort_L(uint L_max, std::vector<uint> &N_sz) {
  ...
  outfile = std::unique_ptr<H5::H5File>(
            new H5::H5File(outfile_name, H5F_ACC_TRUNC));
  ei = std::unique_ptr<H5::DataSet>(new H5::DataSet(
        outfile->createDataSet("idx", H5::PredType::NATIVE_UINT32, 
...
int DMX2e::sort_L(int L_max, std::string dir) {
  ...
  int max_Nsz = *std::max_element(std::begin(cfgs), std::end(cfgs));
  en12.reserve(max_Nsz*2);
  cfg_line cf_l;
  en_data en_d;
  idx4 idx_elm;
  ...
  for(auto i=0; i<ncf; ++i) {
    min_n2 = cfgs[i*5+3]-1;
    max_n2 = cfgs[i*5+4];
    cf_l = {cfgs[i*5],cfgs[i*5+1],cfgs[i*5+2],min_n2,max_n2};
    cfg_st.emplace_back(cf_l);
    t_sz += max_n2-min_n2;
  }
  ...
  std::sort(std::execution::par_unseq, cfg_st.begin(), cfg_st.end(), 
        [](cfg_line const &a, cfg_line const &b) { return a.l1 < b.l1; });
  ...
  for(const auto &line : cfg_st) {
    ...
    if(l2!=last_l2) {
      filename = pot + std::to_string(l2)+std::to_string(l2+1) + gauge + ".h5";
      file = std::unique_ptr<H5::H5File>(new H5::H5File(filename, H5F_ACC_RDONLY));
      e2 = std::unique_ptr<H5::DataSet>(new H5::DataSet(file->openDataSet("e_i")));
      e_space = e2->getSpace();
      e_space.selectHyperslab(H5S_SELECT_SET, count2, offset, stride, block);
      e2->read(&en12[max_Nsz], H5::PredType::NATIVE_DOUBLE, memspace_l2, e_space);
    }
  ...
  max_Nsz = *std::max_element(std::begin(cfgs), std::end(cfgs));
  en12.reserve(max_Nsz*2);
  count2[0] =max_Nsz;
  dimms2[0] =max_Nsz;
  memspace_l2.setExtentSimple(1, dimms2, NULL);
  ...
  for(int L_itr=1; L_itr<L_max; ++L_itr) {
    ...
    cfg::ReadCfg(dir, L_itr+1, sym, ncf, cfgs);
    ...
    // write indices to file
    outfile_name = pot + std::to_string(L_itr+1) + "idx.h5";
    outfile = std::unique_ptr<H5::H5File>(
              new H5::H5File(outfile_name, H5F_ACC_TRUNC));
    ei = std::unique_ptr<H5::DataSet>(new H5::DataSet(
          outfile->createDataSet("idx", H5::PredType::NATIVE_UINT32, 
...
int DMX2e::calc_dmx(int L_max, std::string dir) {
  ...
  int max_N = *std::max_element(cfgs.begin(), cfgs.end());
  int max_Nsz = ncf*max_N;
...
DMX2e::DMX2e(std::string cpot, char gau, uint L_max, std::string inp_dir) {
  pot = cpot;
  gauge = gau;
