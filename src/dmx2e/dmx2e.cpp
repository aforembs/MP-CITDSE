...
int dmx2e::ReadConfig(std::string file, std::string &pot, 
...
int dmx2e::SortL(std::string cpot, int L_max, char gauge, std::string dir) {
  ...
  outfile_name = cpot + "2_01" + gauge + ".h5";
  outfile = std::make_unique<H5::H5File>(
            H5::H5File(outfile_name, H5F_ACC_RDWR));
  ei = std::make_unique<H5::DataSet>(H5::DataSet(
  ...
  for(int L_itr=2; L_itr<=L_max; ++L_itr) {
    outfile_name = cpot + "2_" + std::to_string(L_itr-1) + std::to_string(L_itr) + gauge + ".h5";
    outfile = std::make_unique<H5::H5File>(
              H5::H5File(outfile_name, H5F_ACC_TRUNC));
    ei = std::make_unique<H5::DataSet>(H5::DataSet(
          outfile->createDataSet("e_i", H5::PredType::NATIVE_DOUBLE, 
...
int dmx2e::GenDipole(std::string cpot, int L_max, int l_m, char gauge, std::string dir) {
  ...
  for(int idLf=0; idLf<Lf_sz; ++idLf) { // up to sz Lf
    ...
    for(int idLi=0; idLi<Li_sz; ++idLi) { // up to sz Li
      ...
      if(l1i+1==l1f && l2i==l2f) {
        T[idLi+idLf*Li_sz] = Lsq*pow(-1,l2i+l1f)*sqrt(4*l1f*l1f-1)*
              wig6jj(0,2,2,2*l1f,2*l1i,2*l2i)* //wigner_6j_2e(L,l1f,l1i,l2i)*
              D_data[l1i*max2 + Li_idx[idLi].n1+max_N*Lf_idx[idLf].n1]/sqrt(l1f);
      } else if(l2i+1==l2f && l1i==l1f) {
        T[idLi+idLf*Li_sz] = Lsq*pow(-1,l1i+l2f)*sqrt(4*l2f*l2f-1)*
              wig6jj(0,2,2,2*l2f,2*l2i,2*l1i)* //wigner_6j_2e(L,l2f,l2i,l1i)*
  ...
  // Save TL1;L2
  outfile_name = cpot + "2_" + std::to_string(L) + std::to_string(Lf_i) + gauge + ".h5";
  outfile = std::make_unique<H5::H5File>(
            H5::H5File(outfile_name, H5F_ACC_RDWR));
  ...
  for(Lf_i=2; Lf_i<=L_max; ++Lf_i) {
    ...
    for(int idLf=0; idLf<Lf_sz; ++idLf) { // up to sz Lf
      ...
      for(int idLi=0; idLi<Li_sz; ++idLi) { // up to sz Li
        ...
        if(l1i+1==l1f && l2i==l2f) {
          T[idLi+idLf*Li_sz] = Lsq*pow(-1,l2i+l1f)*sqrt(4*l1f*l1f-1)*
              wig6jj(L2,L2+2,2,2*l1f,2*l1i,2*l2i)* //wigner_6j_2e(L,l1f,l1i,l2i)*
              D_data[l1i*max2 + buffs.at(buf_Li)[idLi].n1+max_N*buffs.at(buf_Lf)[idLf].n1]/sqrt(l1f);
        } else if(l2i+1==l2f && l1i==l1f) {
          T[idLi+idLf*Li_sz] = Lsq*pow(-1,l1i+l2f)*sqrt(4*l2f*l2f-1)*
              wig6jj(L2,L2+2,2,2*l2f,2*l2i,2*l1i)* //wigner_6j_2e(L,l2f,l2i,l1i)*
