...
int main(int argc, char *argv[]) {
  ...
  ens.push_back(std::make_unique<std::vector<double>>(std::vector<double>()));
  ...
  tdrd::readEns(file_prefix, L_max, ct_sz, state_sz, offs, ens);
  ...
  td2e::prop(file_prefix, L_max, t, dt, steps, pulse::SineA_Setup,
             pulse::SineA_A, w, Io, cepd, cycles, ct_sz, offs, state_sz, ens,
             dipoles, ct);
