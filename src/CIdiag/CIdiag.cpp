...
int CalcCI(std::string pot, char gauge, int L_max) {
  ...
  for (auto L = 0; L < L_max; ++L) {
    ...
    // std::cout << LAPACKE_dsyevx(LAPACK_ROW_MAJOR, 'V', 'I', 'U', L_sz,
    // &v12[0], L_sz,
  ...
  for (auto i = 0; i < L_sz; ++i) {
    ...
    dat_fl
        ...
        << std::abs(v12[(2 * L_sz - i - 1) * i / 2 + i]) << "\n";
    vecs[i * L_sz + i] = v12[(2 * L_sz - i - 1) * i / 2 + i];
    // std::cout <<L_idx[i].n1<<" "<<L_idx[i].l1<<" "<<L_idx[i].n2<<" "
    //           <<L_idx[i].l2<<" "<< ens[i] <<"\n";
    for (auto j = i + 1; j < L_sz; ++j) {
      vecs[i * L_sz + j] = v12[(2 * L_sz - i - 1) * i / 2 + j];
      dat_fl
  ...
  std::cout << LAPACKE_dsyevd(LAPACK_ROW_MAJOR, 'V', 'U', L_sz, &vecs[0], L_sz,
                              &eig[0])
            << "\n";
