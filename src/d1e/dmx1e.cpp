...
int dmx1e::readConfig(std::string file, std::string& pot, int& qsz, char& gauge, int& l_max) {
  ...
  std::cout << "No. of outer quadrature points:              " << qsz << std::endl;
...
int dmx1e::genDipole(std::string pot, int qsz, char gauge, int l_max) {
  ...
  auto rset = std::make_unique<H5::DataSet>(H5::DataSet(file->openDataSet("Pr_o")));
...
#pragma omp single
        {
          ...
          file = std::make_unique<H5::H5File>(H5::H5File(filename, H5F_ACC_RDONLY));
          rset = std::make_unique<H5::DataSet>(H5::DataSet(file->openDataSet("Pr_p")));
          rset->read(wfnp.data(), H5::PredType::NATIVE_DOUBLE);
          file->close();
        }
        lp1 = l + 1;
        kl = sqrt(static_cast<double>(lp1 * lp1) / (4.0 * lp1 * lp1 - 1));
        for (int n1 = 0; n1 < nen; ++n1) {
#pragma omp for private(t_ab)
          for (int n2 = 0; n2 < nen; ++n2) {
            t_ab = kl * dmx_int::tvelGL(qsz, lc_sz, n1, l, n2, lp1, qx, qw, wfn, wfnp);
            D[n2 + nen * n1] = t_ab;
          }
        }
#pragma omp single
        {
          ...
          D_set = std::make_unique<H5::DataSet>(H5::DataSet(
              file->createDataSet("d_if", H5::PredType::NATIVE_DOUBLE, H5::DataSpace(2, d_dim))));
