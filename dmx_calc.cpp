...
int DMX2e::sort_L(en_L &Lif, uint N_sz) {
  ...
  uint t_sz=0;
  for(auto i=0; i<Lif.l_pair.size(); ++i) {
    if (l1!=l2) {
      t_sz+=N_sz*N_sz;
    } else {
      t_sz+=N_sz*(N_sz+1)/2;
    ...
    if(l1==l2) {
      ...
      for(uint n1=0; n1<N_sz; ++n1) {
        for(uint n2=n1; n2<N_sz; ++n2) {
          ent = en12[n1] + en12[n2];
          Lif.en_dat.emplace_back((en_data){ent,n1,l1,n2,l1});
        }
      }
      ...
      for(uint n1=0; n1<N_sz; ++n1) {
        for(uint n2=0; n2<N_sz; ++n2) {
          ent = en12[n1] + en12[n2];
          Lif.en_dat.emplace_back((en_data){ent,n1,l1,n2,l2});
        }
      }
    }
  }

  std::sort(std::parallel::par_unseq, Lif.en_dat.begin(), Lif.en_dat.end(), 
        [](en_data const &a, en_data const &b) { return a.en < b.en; });

  Lif.en_dat.erase(Lif.en_dat.begin()+N_sz, Lif.en_dat.end());

  auto max = std::max_element(Lif.en_dat.begin(), Lif.en_dat.end(),
    [](en_data const &a, en_data const &b) { return a.n1 < b.n1; });

  Lif.max_n = std::max(max->n1, std::max_element(Lif.en_dat.begin(), Lif.en_dat.end(),
    [](en_data const &a, en_data const &b) { return a.n2 < b.n2; })->n2);

  outfile_name = pot + std::to_string(Lif.L) + std::to_string(Lif.L+1) + gauge + ".h5";
  outfile = new H5::H5File(outfile_name, H5F_ACC_TRUNC);
  ei = outfile->createDataSet("e_i", H5::PredType::NATIVE_DOUBLE, H5::DataSpace(1, {(hsize_t)N_sz});
  ei.write(&Lif.en_dat[0], H5::PredType::NATIVE_DOUBLE);
  outfile->close();
